...
FCSudoku::FCSudoku(int& it) : it(it) {}
...
bool FCSudoku::Recursive(int r, int c) {
  // find next grid
  while (true) {
    if (puzzle[r][c] == 0) {
      break;
...
bool FCSudoku::Push(int r, int c, int n, int changes_row[10],
...
void FCSudoku::Pop(int r, int c, int n, int changes_row[10],
                   int changes_col[10], int changes_grid[10]) {
  // restore row
...
void FCSudoku::ImportPuzzle(const char* input) {
...
void FCSudoku::ExportResult(const char* output) {
